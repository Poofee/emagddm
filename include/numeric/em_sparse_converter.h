...
public:
    ...
    /**
     * @brief 将CsrMatrix转换为Eigen稀疏矩阵（复数版本）
     * @param csr 输入的CSR格式复数矩阵（const引用）
     * @return Eigen::SparseMatrix<std::complex<double>> 转换后的Eigen稀疏矩阵
     * @throw std::invalid_argument 矩阵未构建或维度不合法时抛出
     * @note 算法步骤：
     *       1. 调用validate_matrix_dimensions_complex()校验输入合法性
     *       2. 提取CSR内部数据(row_ptr, col_indices, values)
     *       3. 使用Eigen::Triplets构建稀疏矩阵
     *       4. 统计转换耗时并记录到ConversionStats
     * @note 性能优化：
     ...
     *       - 使用移动语义减少拷贝开销
     */
    static Eigen::SparseMatrix<std::complex<double>> to_eigen_complex(const CsrMatrix<std::complex<double>>& csr);

    /**
     * @brief 将Eigen稀疏矩阵转换为CsrMatrix（复数版本）
     * @param eigen_mat 输入的Eigen稀疏复数矩阵（const引用）
     * @return CsrMatrix<std::complex<double>> 转换后的CSR格式矩阵
     * @throw std::invalid_argument Eigen矩阵为空或维度不合法时抛出
     * @note 算法步骤：
     *       1. 校验Eigen矩阵是否已压缩且维度合法
     *       2. 提取Eigen内部CSC格式数据(outer_ptr, inner_idx, values)
     ...
     * @note Eigen默认使用CSC(压缩列存储)格式，本方法会正确转换为CSR格式
     */
    static CsrMatrix<std::complex<double>> from_eigen_complex(const Eigen::SparseMatrix<std::complex<double>>& eigen_mat);
    ...
    /**
     * @brief 校验复数矩阵维度合法性（复数版本）
     * @param csr 待校验的CSR复数矩阵
     * @param expected_rows 期望行数（-1表示不检查）
     * @param expected_cols 期望列数（-1表示不检查）
     * @throw std::invalid_argument 以下情况抛出异常：
     *        - 矩阵未构建(is_built()==false)
     *        - 行数或列数<=0
     *        - 提供了expected_rows/expected_cols但维度不匹配
     * @note 建议在to_eigen_complex()/from_eigen_complex()前手动调用以提前发现错误
     */
    static void validate_matrix_dimensions(const CsrMatrix<std::complex<double>>& csr,
                                          int expected_rows = -1,
                                          int expected_cols = -1);
...
private:
    ...
    /**
     * @brief 将Eigen的CSC格式转换为CSR格式（私有辅助方法，复数版本）
     * @param eigen_mat 输入的Eigen稀疏矩阵（需已压缩）
     * @param row_ptr 输出行偏移数组
     * @param col_indices 输出列索引数组
     * @param values 输出数值数组
     * @note Eigen默认使用CSC存储，此方法实现CSC到CSR的高效转换
     */
    static void convert_csc_to_csr_complex(const Eigen::SparseMatrix<std::complex<double>>& eigen_mat,
                                           std::vector<int>& row_ptr,
                                           std::vector<int>& col_indices,
