/**
 * @file electrostatic_solver.hpp
 * @brief 求解器层 - 静电场求解器头文件
 * @details 实现完整的静电场有限元求解流程，对标ANSYS Maxwell Electrostatic Solver。
 *          支持二维/三维/轴对称模型，Dirichlet/Neumann/混合边界条件，
 *          电压源激励，电容矩阵计算，静电力计算等功能。
 *
 *          控制方程：
 *          - 二维/三维：∇·(ε∇φ) = -ρ
 *          - 轴对称（r-z坐标系）：1/r · ∂/∂r(r·ε·∂φ/∂r) + ∂/∂z(ε·∂φ/∂z) = -ρ
 *
 *          弱形式：
 ...
 * @brief 静电场求解器
 * @details 实现完整的静电场有限元求解流程，继承自PhysicsField抽象基类。
 *          核心求解流程：
 *          1. setup(): 前处理检查→材料映射→DOF分配→矩阵装配→边界条件→激励
 *          2. solve(): 选择线性求解器→求解Kφ=F→解向量回扩
 *          3. postProcess(): 计算电场/电位移/能量→存储到FieldData
 */
class ElectrostaticSolver : public PhysicsField {
public:
    ElectrostaticSolver();
    explicit ElectrostaticSolver(tool::DimType dim_type);
    ~ElectrostaticSolver() override = default;
...
private:
    ...
    std::vector<numeric::MaterialProperties> material_list_; ///< 按material_id索引的材料列表
    ...
    /**
     * @brief 从tool::Material构建MaterialProperties映射
